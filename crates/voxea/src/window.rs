...
impl Window {
    ...
    pub fn new(
        ...
        let surface = render_context.create_surface(window.clone()).expect("Could not create surface for window!");
        ...
        let mut config = surface.get_default_config(&render_context.adapter, width, height).unwrap();
        // config.alpha_mode = wgpu::CompositeAlphaMode::PreMultiplied;
        info!("{:?}", config);

        surface.configure(&render_context.device, &config);

        let fbo = render_context.device.create_texture(&wgpu::TextureDescriptor {
        ...
        let fbo_id = render_context.renderer.register_native_texture(&render_context.device, &fbo_view, wgpu::FilterMode::Linear);
        ...
        F: FnMut(&egui::Context),
    {
        ...
        for (id, image_delta) in &full_output.textures_delta.set {
            render_context.renderer
                .update_texture(&render_context.device, &render_context.queue, *id, image_delta);
        }
        ...
        // Renders egui interface on top of framebuffer
        {
            ...
            render_context.renderer.render(&mut rpass, &clipped_primitives, &desc);
        }
